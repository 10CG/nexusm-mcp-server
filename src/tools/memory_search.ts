...
import { McpErrorCode, NexusError, mapSdkErrorToMcpError } from '../errors.js';
...
export const memorySearchTool: ToolDefinition = {
  ...
  handler: async (args) => {
    ...
    // Wave 2B mid_audit-to-pre_merge fix: wrap SDK call in try/catch so
    // 401/403/429 surface as Unauthorized/RateLimited instead of InternalError.
    // nexusm-mcp-server#32: the catch used to recognise only axios-shaped
    // errors, which `@nexusm/sdk` never throws — every real failure fell to
    // the "network" branch. All SDK errors now go through the single bridge.
    let result;
    try {
